...
- **左键点** 🤖 → 弹出面板：
  - **5 小时窗口** 和 **7 天窗口** 进度条（填充=已用，绿→黄→橙→红）、**已用%**、**预计更新时间**，
    以及**按当前速度到重置时的预计已用%**
  - 如果你的套餐有**按模型的周限额**（比如 **Fable**），也会多显示一条 —— 模型名直接取自
    Anthropic，改名会自动跟随
  - **🔄 刷新** —— 重新拉官方限额
  - **📊 展开为看板 →** —— 用浏览器打开完整 HTML 看板
- **右键点** 🤖 → 小菜单：**刷新限额** / **展开为看板** /
  **语言（Language）** / **开机自启** / **检查更新** / **限额提醒** / **退出**。
...
更新检查**默认关闭**——出于隐私，首次启动会**问你一次**是否开启，之后随时可在右键菜单
**检查更新**里开关。开启后，app 会去 GitHub 查有没有新版本（只发一个版本号请求、不发送你的任何数据）；
有新版就弹通知、菜单栏图标加个 **•**、菜单里出现**更新**项。点它会打开仓库页；更新方式 =
`git pull` 后重新跑 `bash build_menubar.command`。

### 限额提醒

如果你的套餐有**按模型的周限额**（比如 Fable），app 会在它越过 **50%**、**75%**，之后**每 +5%**
（80/85/90/95/100）时各弹一次通知；某天用掉超过该周限额的 **20%** 也提醒一次。边沿触发（每档只弹一次、不刷屏），
周重置后重新武装。**默认开**，右键菜单 → **限额提醒** 可关。
...
很小。工具**只读** Claude Code 自己的 `~/.claude/projects/**/*.jsonl` 会话日志（不创建、不修改），
唯一写入的是一个小 JSON 缓存 `~/.claude/usage-peek-quota.json` —— 只有几百字节，存的是
百分比、重置时间、套餐名（**不含任何对话内容**）。没有数据库、没有常驻后台进程；菜单栏 app
平时空闲、每两分钟刷新一次，CPU / 内存 / 磁盘占用都可忽略。
...
- 菜单栏 app 只读限额缓存（`~/.claude/usage-peek-quota.json`）并调用自带的 Python 脚本。
  它唯一可能的联网是**可选、默认关闭**的更新检查（向 GitHub GET 一个版本号、不发送任何数据）——
  你不开它就完全不联网。
